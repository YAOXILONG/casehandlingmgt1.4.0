<template>
  <watermarkContent>
    <div class="returnOfGoodsIndex">
      <div class="returnOfGoodsIndexBox">
        <FilterSearchBox>
          <FilterSearchItem>
          ...
          <FilterSearchItem v-show="$checkPlace">
            ...
            <div class="item-input">
              <el-select v-model="searchForm.bandNo" placeholder="点击此文本框后刷手环" clear>
                <el-option
        ...
        <FilterSearchContent>
          <div class="returnOfGoodsIndexTable">
            <div class="table-container">
              ...
              >
                ...
                <el-table-column label="性别" show-overflow-title width="90">
                  <template slot-scope="scope">
                ...
                <el-table-column
                  ...
                  prop="areaName"
                ></el-table-column>
                <el-table-column label="入所时间" show-overflow-title>
                  <template slot-scope="scope">
                    {{
                      scope.row.entryTime
                ...
                <el-table-column label="入所时长" show-overflow-title>
                  <template slot-scope="scope">
                ...
                >
                  <template slot-scope="scope">
                    <!-- <span :class="scope.row.doBodyCheck ? 'greenSpan' : 'redSpan'">检</span>
                  <span :class="scope.row.doGoodsStorage ? 'greenSpan' : 'redSpan'">物</span>
                  <span
                    v-if="Number(scope.row.personType) === 1"
                    :class="scope.row.doCollected ? 'greenSpan' : 'redSpan'"
                  >采</span>
                  <span
                    v-if="Number(scope.row.personType) === 1"
                    :class="scope.row.doDrugCheck ? 'greenSpan' : 'redSpan'"
                  >毒</span> -->
                    <span
                ...
                <el-table-column label="操作" width="120">
                  <template slot-scope="scope">
                    <span
                      ...
                      class="operationBtn"
                      v-if="
                        scope.row.doGoodsReturn === 0 &&
                        scope.row.doGoodsStorage == 1
                      "
        ...
        _self.websock.onmessage = async (data) => {
          ...
          if (
            ...
            for (let i = 0; i < this.bandsList.length; i++) {
              if (this.bandsList[i].tagCode === CardCode) {
                string = CardCode
                break
              }
            }
            if (string) {
              if (this.searchForm.bandNo === string) {
                this.searchForm.bandNo = string
              } else {
                this.searchForm.bandNo = string
                this.handleSearch()
              }
...
.returnOfGoodsIndex {
  ...
  .returnOfGoodsIndexBox {
    width: 100%;
    height: ~'calc(100% - 57px)';
    > .filter-search-content {
      .returnOfGoodsIndexTable {
        height: 100%;
        width: 100%;
      }
      .table-container {
