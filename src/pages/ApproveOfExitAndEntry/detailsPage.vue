<template>
  <watermarkContent>
    <div class="applicationofApprove-detail-box add-edit-from-box full">
          ...
          <div class="leftTableInfo">
            ...
            <div class="tableContent clearfix">
              <ul class="applicationDetailSituationTable clearfix">
                ...
                <el-col :span="24" v-if="currentPerson.applyType == 2">
                  <li class="tableItem">
                ...
                <el-col :span="12" v-if="currentPerson.applyType == 2">
                  <li class="tableItem">
                ...
                <el-col :span="12" v-if="currentPerson.applyType == 3 ||currentPerson.applyType == 4">
                  <li class="tableItem">
                ...
                <el-col :span="12">
                  <li class="tableItem">
                    <div class="tableItemLabel">申请类型</div>
                    <div
                      class="tableItemCon"
                      :title="currentPerson.applyType || ''"
                    >
                      <span v-if="currentPerson.applyType == 1">入区预约</span>
                      <span v-if="currentPerson.applyType == 2">入区申请</span>
                      <span v-if="currentPerson.applyType == 3">出区申请</span>
                      <span v-if="currentPerson.applyType == 4"
                        >临时出区申请</span
                      >
                    </div>
                  </li>
                ...
                <el-col :span="12">
                  <li class="tableItem">
                    <div class="tableItemLabel" v-if="currentPerson.applyType ==3 ||currentPerson.applyType ==4">出区原由</div>
                    <div class="tableItemLabel" v-if="currentPerson.applyType ==1 ||currentPerson.applyType ==2">入区原由</div>
                    <div
                      class="tableItemCon"
                      v-if="currentPerson.applyType == 3"
                      :title="handleResultMap[currentPerson.applyReason]|| ''"
                    >
                      {{ handleResultMap[currentPerson.applyReason]|| '' }}
                    </div>
                    <div
                      class="tableItemCon"
                      v-if="currentPerson.applyType == 4"
                      :title="temporaryLeaveReasonMap[currentPerson.applyReason] || ''"
                    >
                      {{ temporaryLeaveReasonMap[currentPerson.applyReason]|| '' }}
                    </div>
                    <div
                      class="tableItemCon"
                      v-if="currentPerson.applyType == 2"
            ...
            <el-steps :space="150" direction="vertical" :active="active5">
              ...
              >
                ...
                <template slot="description">
                  <div>
                    {{
                      $moment(item.operatingTime).format("YYYY-MM-DD HH:mm:ss")
                    }}
                  </div>
      ...
      <div class="form-btns" v-if="currentPerson.applyStatus == 0">
        <div class ="form-btns-center" style="width:85%">
          <el-button class="add-edit-btn" type="primary" @click="agreeHandle"
          v-if="currentPerson.approvalUser == currentUser"
            >同意</el-button
          >
          <el-button
            class="add-edit-btn"
...
export default {
  name: 'approveOfExitAndEntryDetails',
  data() {
    return {
      active5: 2,
      pathDescArr: ['出入区审批', `${this.$route.query.id ? '详情' : ''}`],
      inqDictList: [],
      applyIndexCode: this.$route.query.id,
      currentUser: this.$route.query.currentUser,
      currentPerson: {},
      currentPersonApplicaitionInfo: {},
      idcardTypeMap: {},
      registerTypeMap: {},
      personTypeMap: {},
      nationalityMap: {},
  ...
  methods: {
    ...
    //   };
    //   try {
    //     let res = await server.approvalSearch(params);
    //     if (res.data.code === "0") {
    //       let data = res.data.data;
    //       if (data) {
    //         this.currentPersonApplicaitionInfo = JSON.parse(
    //           JSON.stringify(data.list[0])
    //         );
    //       } else {
    //         this.currentPersonApplicaitionInfo = {};
    //         this.$message({
    //           type: "warning",
    //           message: "暂无人员信息",
    //         });
...
.applicationofApprove-detail-box {
  min-width: 1366px;
  height: 100%;
  overflow: hidden;
  color: #4d4d4d;
