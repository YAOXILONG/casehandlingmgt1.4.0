<template>
  <el-dialog :class="['shuttlebox_dialog', {single: !multiple}]"
             :area="[991, 636]"
             no-scrollbar
             title="人员选择"
    ...
    <div class="ShuttleBox">
      <div class="sider">
        ...
        <div class="tree-wrap">
          <el-tree default-icon="icon-department"
      ...
      <div class="left">
        ...
        <div class="left-search">
          <el-input clearable
                    ...
                    :clear-icon-click="leftClearIconClick"></el-input>
        </div>
        <div class="left-table">
          <div class="left-table-cantainer">
            <el-table force-scroll
                      style="width: 100%"
              ...
              <el-table-column show-overflow-title
                               :label="multiple?'全选': '人员'">
                <template slot-scope="scope">
      ...
      <template v-if="multiple">
        <div class="middle">
          <el-button icon-border="h-icon-angle_right"
                     :disabled="!leftMultipleSelection.length"
                     @click="pushData"></el-button>
          <el-button icon-border="h-icon-angle_left"
        ...
        <div class="right">
          ...
          <div class="right-search">
            <el-input clearable
                      ...
                      :clear-icon-click="rightClearIconClick"></el-input>
          </div>
            ...
            <div class="right-table-cantainer">
              <el-table force-scroll
                ...
                <el-table-column show-overflow-title
                                 label="全选">
                  <template slot-scope="scope">
                    {{ `${scope.row.personName}(${scope.row.certificateNumber || '无证件号'})--${scope.row.treePath}` }}
                  </template>
                </el-table-column>
              </el-table>
            </div>
...
export default {
  ...
  methods: {
    ...
    // 树
    TreeHandleIconClick () {
      const first = getFirstObjectInTree(this.treeData, this.filterText, defaultProps.label)
      console.log(first[defaultProps.id])
      if (first && first[defaultProps.id]) {
        this.filterNothing = false
        this.$nextTick(() => {
          this.TreeHandleNodeClick(first)
          this.$refs.tree && this.$refs.tree.setCurrentKey(first[defaultProps.id])
    ...
    setCurrent (id) {
      ...
      this.$refs.leftMultipleTable && this.$refs.leftMultipleTable.setCurrentRow(row[0])
    },
    selectable (column) {
