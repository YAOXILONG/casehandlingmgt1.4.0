<template>
  <div class="filter-search-box-applicate">
...
<script>
export default {
  name: 'FilterSearchBoxForApplicate',
  props: {
    isFilter: {
      type: Boolean,
      default: false,
  ...
  methods: {
    ...
    searchBoxInit() {
      let searchBox = document.getElementsByClassName('filter-search-box-applicate')
      ...
      this.$nextTick(() => {
        ...
        if(this.isFilterBox){
          ...
          filterBoxHeight && filterBoxHeight + 20 + 0
        }px)`
        } else {
          contentBox[0].style.height = `calc(100% - ${
